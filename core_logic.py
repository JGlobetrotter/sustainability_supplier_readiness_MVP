...
if "respondent_email" not in st.session_state:
...
email_input = st.text_input(
    "Your email address *",
    value=st.session_state.respondent_email,
    placeholder="name@company.com",
    key="email_field",
)
st.session_state.respondent_email = email_input
st.markdown("")
...
if run:
    # Validate email
    email = st.session_state.respondent_email.strip()
    if not email or "@" not in email or "." not in email.split("@")[-1]:
        st.warning("Please enter a valid email address before running.")
    else:
...
if st.session_state.results is None:
    ...
    if email:
        st.caption(f"📧 Submitted by: **{email}**")
